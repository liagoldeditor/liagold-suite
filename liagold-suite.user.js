...
// @version      1.0.21
// @description  v1.0.21: Unified Suite - Totalizer, Scanner, Payment Detail & ERP Footer
...
(() => {
  ...
  function ensureHeaderColumns(table) {
    ...
    if (!headerRow.querySelector(`th.${METHOD_HEADER_CLASS}`)) {
      ...
      headerRow.insertBefore(th, noteTh);
    }
    ...
    if (!headerRow.querySelector(`th.${AMOUNT_HEADER_CLASS}`)) {
      ...
      const next = noteTh.nextElementSibling;
      headerRow.insertBefore(th, next || null);
    }
  ...
  function ensureRowCells(row) {
    ...
    if (!row.querySelector(`td.${AMOUNT_CELL_CLASS}`)) {
      ...
      const next = noteCell.nextElementSibling;
      row.insertBefore(td, next || null);
    }
  }
  ...
  function fetchPayment(code) {
    ...
    const promise = (async () => {
      try {
        let json = await fetchJson(buildApiUrl(code));
        ...
        if (!item) {
          ...
          if (digits && digits !== code) {
            json = await fetchJson(buildApiUrl(digits));
